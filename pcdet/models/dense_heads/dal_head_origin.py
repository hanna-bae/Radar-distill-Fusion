...
class DALHead(TransFusionHead):
    ...
    def forward_single(self, inputs, img_inputs, bev_feat_img=None):
        ...
        # dense_fuse_feat 확인 
        # print("dense_fuse_feat.shape: ", dense_fuse_feat.shape)
        dense_heatmap = self.heatmap_head(dense_fuse_feat)
        heatmap = dense_heatmap.detach().sigmoid()
        # print('dense_heatmap:', dense_heatmap)
        # def project_gt_boxes_to_bev(gt_boxes, pc_range, voxel_size, out_size_factor):
        #     """
        #     Args:
        #         gt_boxes: torch.Tensor of shape [num_gt, 7]  # [x, y, z, dx, dy, dz, yaw]
        #         pc_range: List[6] → [x_min, y_min, z_min, x_max, y_max, z_max]
        #         voxel_size: List[3] → [vx, vy, vz]
        #         out_size_factor: int (downsample rate of the BEV feature map)
        #     Returns:
        #         gt_centers_bev: torch.Tensor of shape [num_gt, 2]  # [x_grid, y_grid]
        #     """
        #     x_min, y_min = pc_range[0], pc_range[1]
        #     vx, vy = voxel_size[0], voxel_size[1]

        #     centers = gt_boxes[:, :2]  # [x, y]
        #     centers_x = (centers[:, 0] - x_min) / (vx * out_size_factor)
        #     centers_y = (centers[:, 1] - y_min) / (vy * out_size_factor)
            
        #     gt_centers_bev = torch.stack([centers_x, centers_y], dim=-1).round().long()  # shape: [N, 2]
        #     return gt_centers_bev

        # # 3. Top-N 추출 함수
        # def extract_topn_coords(heatmap, N=100):
        #     B, K, H, W = heatmap.shape
        #     flat = heatmap.view(B, K, -1)  # [B, K, H*W]
        #     topk_scores, topk_idx = torch.topk(flat, N, dim=-1)  # [B, K, N]
        #     coords = torch.stack([topk_idx // W, topk_idx % W], dim=-1)  # [B, K, N, 2]
        #     return coords[0]  # shape: [K, N, 2]

        # # 4. Recall 계산 함수
        # def compute_recall(gt_centers, top_coords, radius=2):
        #     matched = 0
        #     for gt in gt_centers:
        #         for cls_coords in top_coords:
        #             dists = (cls_coords - gt).abs()
        #             if (dists <= radius).all(dim=1).any():
        #                 matched += 1
        #                 break
        #     return matched / len(gt_centers)
        # # heatmap test 
        # gt_boxes = batch_data['gt_boxes'][0]  # 예: [N, 7]

        # gt_centers = project_gt_boxes_to_bev(
        #     gt_boxes=gt_boxes,
        #     pc_range=self.bbox_coder.pc_range,
        #     voxel_size=self.bbox_coder.voxel_size,
        #     out_size_factor=self.bbox_coder.out_size_factor
        # )

        # topn_coords = extract_topn_coord(heatmap, N=50)
        # recall = compute_recall(gt_centers, topn_coords)
        # print(f"[Heatmap evaluation] Recall@100: {recall: .3f}")
