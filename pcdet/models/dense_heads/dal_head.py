...
class DALHead(TransFusionHead):
    def __init__(self,
        ...
        cfg = dict(
            type='CustomResNet',
            numC_input=self.hidden_channel + feat_bev_img_dim,
            num_layer=[dense_fuse_layers+1, ],
            num_channels=[self.hidden_channel, ],
            stride=[1, ],
            backbone_output_ids=[0, ])
        
        self.dense_heatmap_fuse_convs = CustomResNet(cfg)
    ...
    def extract_img_feat_from_3dpoints(self, points, img_inputs_list, fuse=True):
        ...
        b, p, _ = points.shape
        points = points.view(b, 1, -1, 3, 1).float()
        for img_inputs in img_inputs_list:
            img_feats = img_inputs[0].permute(0, 2, 1, 3, 4).contiguous().float()
            ...
            with torch.no_grad():
                ...
                normalize_factor = torch.tensor([w - 1.0, h - 1.0, n - 1.0]).to(grid.device).float()
    ...
    def forward_single(self, data_dict):
        ...
        """
        batch_size = data_dict['spatial_features'][0]
        radar_feat = data_dict['pillar_features_scattered']
        img_feat = data_dict['spatial_features']
        
        radar_feat = self.shared_conv(radar_feat)
        radar_feat_flatten = radar_feat.view(batch_size, radar_feat.shape[1], -1)

        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(radar_feat.device)

        # predict dense heatmap
        dense_fuse_feat = torch.cat([radar_feat, img_feat],
                                             dim=1)
        dense_fuse_feat = \
        ...
        index = top_proposals_index.expand(-1, radar_feat_flatten.shape[1],
                                           -1)
        ...
        bev_feat_img = img_feat.view(batch_size, img_feat.shape[1], -1)
        index = top_proposals_index.expand(-1, bev_feat_img.shape[1], -1)
        query_feat_img_bev = bev_feat_img.gather(index=index, dim=-1)

        query_feat_fuse = torch.cat([query_feat_radar, query_feat_img_bev], dim=1)
